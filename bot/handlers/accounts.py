...
async def account_select_by_name(message: Message) -> None:
    ...
    await message.answer(
        f"🔑 <b>Аккаунт: {esc(account.name)}</b>\n\n"
        f"ID: {account.id}\n"
        f"Токен: {esc(account.token[:8])}...{esc(account.token[-4:])}",
...
async def account_info(message: Message) -> None:
    ...
    async with WbClient(account.token) as client:
        try:
            ...
            await message.answer(f"❌ Ошибка: {esc(str(e))}", reply_markup=account_actions_kb())
            return

    text = (
        f"ℹ️ <b>Информация об аккаунте</b>\n\n"
        f"Название: {esc(account.name)}\n"
        f"Продавец: {esc(info.get('name', '—'))}\n"
        f"Торговая марка: {esc(info.get('tradeMark', '—'))}\n"
        f"ИНН: {esc(info.get('tin', '—'))}\n"
        f"SID: {esc(info.get('sid', '—'))}"
    )
...
async def account_ignore_list(message: Message) -> None:
    ...
    if items:
        text = (
            f"🚫 <b>Игнор-лист: {esc(account.name)}</b>\n\n"
        ...
        text = f"🚫 <b>Игнор-лист: {esc(account.name)}</b>\n\nИгнор-лист пуст."
